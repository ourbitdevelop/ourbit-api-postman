...
为了便于用户快速方便的使用RESTful APIs，OURBIT提供几个 Postman Collections 和 Environments 文件（JSON文件)。<br/>
只需要导入并设置自己的API密钥就能即可开始。


## 如何导入和配置
- 下载 `ourbit-api-postman` 存储库到本地。
...
- 未设置 `API key`。
- `API key`不正确。
- 在Postman`Headers`选项卡中未选择`x-ourbit-apikey`。
